...
def get_scenario_tasks():
    ...
    if mode == "single_reward_brand":
        from case.Single_interface.reward_brand import CheckRewardBrand

        return [CheckRewardBrand]
    
    if mode == "single_brand_goods":
        from case.Single_interface.gacha_brand_goods import CheckGachaBrandGoods

        return [CheckGachaBrandGoods]
    
    if mode == "single_user_goods_one":
        from case.Single_interface.user_goods_one import CheckUserGoodsOne

        return [CheckUserGoodsOne]
    
    if mode == "single_v3_check_goods":
        from case.Single_interface.gacha_v3_check_goods import CheckGachaV3CheckGoods

        return [CheckGachaV3CheckGoods]
    
    if mode == "single_v3_one_cards":
        from case.Single_interface.gacha_v3_goods_one_cards import CheckGachaV3OneCards

        return [CheckGachaV3OneCards]
    
    if mode == "single_v3_backpack":
        from case.Single_interface.gacha_v3_backpack import CheckGachaV3Backpack

        return [CheckGachaV3Backpack]
    ...
    raise ValueError(
        ...
        "single_pay_result, single_pay_result_lite, single_coupon, single_brand, single_reward_brand, single_brand_goods, single_user_goods_one, single_v3_check_goods, single_v3_one_cards, single_v3_backpack"
    )
