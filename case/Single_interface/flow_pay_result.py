...
def query_pay_result_once(task_set):
    ...
    with task_set.client.get(
        ...
        if response.status_code == 200:
            log_debug(f"pay result query success, orderNo={order_no}")
            response.success()
            return True

        log_error(f"pay result query failed: orderNo={order_no}, status={response.status_code}")
...
class FlowCheckPayResult(SequentialTaskSet):
    wait_time = between(LoadTestConfig.WAIT_MIN, LoadTestConfig.WAIT_MAX)

    @task
    def query_pay_result(self):
        query_pay_result_once(self)


class FlowCheckPayResultLite(SequentialTaskSet):
    wait_time = between(LoadTestConfig.WAIT_MIN, LoadTestConfig.WAIT_MAX)
