...
REWARD_BRAND_LIST_URL = f"{ApiPaths.REWARD_BRAND_LIST}?{urllib.parse.urlencode({'page': '1', 'page_size': '10'})}"




class CheckRewardBrand(SequentialTaskSet):
    ...
    @task
    def check_reward_brand(self):
        # 轻量模式只依赖 Locust 原生 HTTP 统计，不做业务计数和手动响应标记。
        response = self.client.get(
            REWARD_BRAND_LIST_URL,
            headers=self.headers,
            name=ApiPaths.REWARD_BRAND_LIST
        )
