...
GACHA_BRAND_GOODS_LIST_URL = f"{ApiPaths.GACHA_BRAND_GOODS_LIST}?{urllib.parse.urlencode({'page': '1', 'page_size': '10', 'hideCard': '0'})}"




class CheckGachaBrandGoods(SequentialTaskSet):
    ...
    @task
    def check_gacha_brand_goods(self):
        # 轻量模式只依赖 Locust 原生 HTTP 统计，不做业务计数和手动响应标记。
        response = self.client.get(
            GACHA_BRAND_GOODS_LIST_URL,
            headers=self.headers,
            name=ApiPaths.GACHA_BRAND_GOODS_LIST
        )
